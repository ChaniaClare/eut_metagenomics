...
# Intended to follow from Kneaddata
...
if [[ $# -gt 0 ]]; then
    if [[ "$1" == "redo_all" ]]; then
        REDO_ALL=true
        echo "Mode: REDO ALL - Will regenerate all outputs"
    elif [[ "$1" == "skip_if_done" ]]; then
        REDO_ALL=false
        echo "Mode: SKIP IF DONE - Will skip samples with existing outputs"
    else
        echo "Unknown argument: $1"
        echo "Usage: $0 [redo_all|skip_if_done]"
        echo "Default is skip_if_done if no argument provided"
    fi
else
    echo "Mode: SKIP IF DONE (default) - Will skip samples with existing outputs"
fi
...
# Ensure SAM directory exists
SAM_DIR="${PROCESSED_DATA_DIR}/sam"
mkdir -p "${SAM_DIR}"
...
for sample in "${ALL_SAMPLES[@]}"; do
   ...
   # Check if output files already exist and skip if configured to do so
   if [[ "$REDO_ALL" == "false" ]] && \
      [[ -f "${TAXONOMY_DIR}/${sample}/${sample}_profile.txt" ]] && \
